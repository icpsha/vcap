...
class JobManager
  ...
  SERVICES = ["redis", "mysql", "mongodb", "neo4j","maxdb"]
  ...
  INSTALLED_JOB_PROPERTIES = {NATS => ["host"], CC => ["service_api_uri", "builtin_services"],
                              CCDB => ["host"]}
  INSTALL_JOB_PROPERTIES = {CC => ["builtin_services"], MYSQL_NODE => ["index"], MONGODB_NODE => ["index"], REDIS_NODE => ["index"], NEO4J_NODE => ["index"], MAXDB_NODE => ["index"]}
